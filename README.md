...
```bash
./scripts/run/quick-build-deploy.sh --backend --admin --web --business
./scripts/run/quick-build-deploy.sh --web
./scripts/run/quick-build-deploy.sh --business
./redeploy.sh
```

`redeploy.sh` defaults to the Docker-backed backend + admin + web + business stack plus the iOS simulator flow.
...
- `./scripts/deploy/deploy.sh`: runs the full sequence for backend + web + admin + business
...
```bash
./scripts/deploy/deploy.sh --web --admin --business --preview
```
